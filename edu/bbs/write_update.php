...
@mkdir(PORTAL_DATA_PATH.'/file/'.$bo_table, G5_DIR_PERMISSION);
@chmod(PORTAL_DATA_PATH.'/file/'.$bo_table, G5_DIR_PERMISSION);
...
if(isset($_FILES['bf_file']['name']) && is_array($_FILES['bf_file']['name'])) {
    for ($i=0; $i<count($_FILES['bf_file']['name']); $i++) {
        ...
        if (isset($_POST['bf_file_del'][$i]) && $_POST['bf_file_del'][$i]) {
            ...
            $delete_file = run_replace('delete_file_path', PORTAL_DATA_PATH.'/file/'.$bo_table.'/'.str_replace('../', '', $row['bf_file']), $row);
        ...
        if (is_uploaded_file($tmp_file)) {
            ...
            if ($w == 'u') {
                ...
                if(isset($row['bf_file']) && $row['bf_file']){
                    ...
                    if( file_exists($delete_file) ){
                        ...
                        @unlink(PORTAL_DATA_PATH.'/file/'.$bo_table.'/'.$row['bf_file']);
                    }
                    // 이미지파일이면 썸네일삭제
                    if(preg_match("/\.({$config['cf_image_extension']})$/i", $row['bf_file'])) {
                        delete_board_thumbnail($bo_table, $row['bf_file']);
            ...
            $dest_file = PORTAL_DATA_PATH.'/file/'.$bo_table.'/'.$upload[$i]['file'];
