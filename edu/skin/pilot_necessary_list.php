...
$sql = " select count(*) as cnt from ".$g5['pilot_necessary_table'];
$row = sql_fetch($sql);
$total_count = $row['cnt'];
...
$sql = " select * from ".$g5['pilot_necessary_table']." order by idx desc limit {$from_record}, {$rows} ";
...
<?php
    $row_apply = sql_fetch(" select * from {$g5['pilot_lecture_apply_table']} where mb_id='{$member['mb_id']}' and lecture_idx = '{$row[idx]}' and lecture_type_table = 'pilot_necessary' ");
...
?>
            ...
            <input type="button" class="btn btn_02" value="<?=$lang['lecture_start']?>"  onclick="location.href='<?=G5_BBS_URL?>/content.php?co_id=lecture_view&idx=<?=$row[idx]?>&type=pilot_necessary'">
<?php
        }else{
    ...
    function apply_chk(subject,idx){
        ...
        if(confirm(subject_tmp)){
            ...
            $.ajax({
                ...
                data		: {
                    "idx"                   : idx,
                    "lecture_type_table"    : "pilot_necessary",
                },
