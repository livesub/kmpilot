...
$sql_order = " where (edu_type = 'CR' OR edu_type = 'CE' OR edu_type = 'CF') and edu_onoff_type = 'off' and edu_del_type ='N' {$search} and edu_regi like '%{$select_y}%' order by edu_idx desc";
...
<div class="btn_fixed_top">
    ...
    <?php } ?>
    <input type="button" class="btn btn_01" value="전체 신청자 리스트" onclick="apply_manage_list('','','off','all');">
</div>
...
<script>
    function apply_manage_list(edu_idx='',edu_type='',edu_onoff_type,choice_type){
        ...
        $("#form_submit").attr("action", "pilot_off_apply_manage_list.php");
        $("#form_submit").submit();
    }
</script>
