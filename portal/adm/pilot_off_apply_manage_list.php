...
{
    if($choice_type == "all") $g5['title'] = '오프라인 교육 전체 신청자 리스트';
    else $g5['title'] = '오프라인 교육 신청자 리스트';
...
<select name="sfl" id="sfl">
    ...
    <option value="edu_name_kr" <?php if($_GET['sfl'] == "edu_name_kr") echo "selected";?>>교육명</option>
...
<form name="off_apply_list_from" id="off_apply_list_from" onsubmit="return off_apply_list_submit(this);" method="post">
...
if($choice_type != "all"){
    //전체 보기에선 기능 막음
?>
    <input type="button" class="btn btn_01" id="" value="수료확인" onclick="edu_choice('complet');">
    <input type="button" class="btn btn_02" id="" value="수료취소" onclick="edu_choice('cancel');">
    <input type="button" class="btn btn_01" id="" value="수강자등록" onclick="edu_apply_pop();">
<?php
}
?>
    <input type="button" class="btn btn_02" id="" value="리스트 다운로드" onclick="excel_down('<?=$edu_idx?>','<?=$edu_type?>');">
</div>
...
<script>
function off_apply_list_submit(f)
{
    if (!is_checked("chk[]")) {
...
<script>
    function edu_choice(choice){
        if(choice == "complet") var ment = "수료 확인";
        else var ment = "수료 취소";

        if (!is_checked("chk[]")) {
            alert(ment + " 하실 항목을 하나 이상 선택하세요.");
            return false;
        }
...
<script>
    function excel_down(edu_idx="",edu_type=""){
        ...
        $("#choice_type_print").val("<?=$choice_type?>");
        $("#edu_onoff_type_print").val("<?=$edu_onoff_type?>");
        $("#form_print").attr("action", "pilot_apply_excel_down.php");
        $('#form_print').attr("target", "");
        $('#form_print').attr("method", "POST");
        $("#form_print").submit();
    }
