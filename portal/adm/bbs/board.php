...
if (isset($write['wr_is_comment']) && $write['wr_is_comment']) {
    goto_url(get_pretty_url_admin($bo_table, $write['wr_parent'], '#c_'.$wr_id));
}
...
if ((isset($wr_id) && $wr_id) || (isset($wr_seo_title) && $wr_seo_title)) {
    ...
    if (!$write['wr_id']) {
        $msg = '글이 존재하지 않습니다.\\n\\n글이 삭제되었거나 이동된 경우입니다.';
        alert($msg, get_pretty_url_admin($bo_table));
    }
    ...
    if (isset($group['gr_use_access']) && $group['gr_use_access']) {
        if ($is_guest) {
            ...
            alert($msg, G5_BBS_URL.'/login.php?wr_id='.$wr_id.$qstr.'&amp;url='.urlencode(get_pretty_url_admin($bo_table, $wr_id, $qstr)));
        }
    ...
    if ($member['mb_level'] < $board['bo_read_level']) {
        if ($is_member)
            ...
            alert('글을 읽을 권한이 없습니다.\\n\\n회원이시라면 로그인 후 이용해 보십시오.', G5_BBS_URL.'/login.php?wr_id='.$wr_id.$qstr.'&amp;url='.urlencode(get_pretty_url_admin($bo_table, $wr_id, $qstr)));
    }
