...
if($w == "")
{
    $sql = " insert {$g5['edu_new_win_table']} set $sql_common ";
    sql_query($sql);
...
{
    $sql = " update {$g5['edu_new_win_table']} set $sql_common where nw_id = '$nw_id' ";
    sql_query($sql);
}
else if ($w == "d")
{
    $sql = " delete from {$g5['edu_new_win_table']} where nw_id = '$nw_id' ";
    sql_query($sql);
}

if ($w == "d")
{
    goto_url('./edu_newwinlist.php');
}
else
{
    goto_url("./edu_newwinform.php?w=u&amp;nw_id=$nw_id");
}