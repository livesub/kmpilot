...
<!--        		<span class="sound_only">조회</span><strong><i class="fa fa-eye" aria-hidden="true"></i> --><?php //echo number_format($view['wr_hit']) ?><!--회</strong>-->
	    ...
	    <div id="bo_v_top">
	        ...
	        <ul class="btn_bo_user bo_v_com">
                ...
                <?php if ($update_href) { ?><li><a href="<?php echo $update_href ?>" title="수정" class="btn_b01 btn"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a></li><?php } ?>
			    <?php if ($delete_href) { ?><li><a href="<?php echo $delete_href ?>" onclick="del(this.href); return false;" title="삭제" class="btn_b01 btn"><i class="fa fa-trash-o" aria-hidden="true"></i></a></li><?php } ?>
...
;
    ...
    if ($view['file']['count']) {
        for ($i=0; $i<count($view['file']); $i++) {
            //&& !$view['file'][$i]['view'] 이거를 아래 비교문에 넣을 시 이미지 첨부파일 숨김 (보이는 사진은 첨부파일에 포함안시킬경우)
    ...
    <section id="bo_v_file">
        ...
        // 가변 파일
        for ($i=0; $i<count($view['file']); $i++) {
            //&& !$view['file'][$i]['view'] 이거를 아래 비교문에 넣을 시 이미지 첨부파일 숨김
            if (isset($view['file'][$i]['source']) && $view['file'][$i]['source']) {
         ?>
            <li>
